...
function SendData(Host: AnsiString; Port: Word; SendString: AnsiString; Timeout: LongWord = 0): Boolean;
function SendDataAndReceiveText(Host: AnsiString; Port: Word; SendString: AnsiString; out ConnectionStatus: Boolean; Timeout: LongWord = 0): AnsiString;
...
begin
  ...
  begin
    ...
    begin
      ...
      SocketInfo.ConnectionStatus := FD_ISSET(SocketInfo.Socket, FDSetW);
    end;
    NonBlockingMode := 0;
    IoCtlSocket(SocketInfo.Socket, FIONBIO, NonBlockingMode);
  end
...
function SendData(Host: AnsiString; Port: Word; SendString: AnsiString; Timeout: LongWord = 0): Boolean;
var
  ClientSocket: TClientSocketTCP;
begin
  ClientSocket := TClientSocketTCP.Create;
  ClientSocket.Timeout := Timeout;
  ClientSocket.ConnectToServer(Host, Port);
  if not ClientSocket.ConnectionStatus then
  begin
    Result := False;
  ...
  Result := True;
  ClientSocket.Send(PAnsiChar(SendString), Length(SendString));
  ClientSocket.Disconnect;
  ClientSocket.Destroy;
end;
...
begin
  ...
  FillChar(Buffer^, BufferSize, #0);
  ClientSocket.Send(PAnsiChar(SendString), Length(SendString));
  Size := Recv(ClientSocket.SocketInfo.Socket, Buffer^, BufferSize, 0);
